...
{
	brillo = 40;
	especular = _vertex4f(0.7, 0.7, 0.7, 1);
	ambiente_difusa = _vertex4f(0.7, 0.7, 0.7, 1);
...
void _triangulos3D::draw(_modo modo, float r1, float g1, float b1, float r2, float g2, float b2, float grosor)
{
	...
	case SOLID_ILLUMINATED_GOURAUD:
		draw_iluminacion_suave(r1,g1,b1);
		break;
	}
}
...
void _triangulos3D::draw_iluminacion_suave(float r, float g, float b)
{
	int i;
	GLfloat ambient_component[4] = {1,1,1,1};
	GLfloat material[4] = {r, g, b, 1};
...
void _rotacion::parametros(vector<_vertex3f> perfil, int num, int tapa)
{
	...
	for (j = 0; j < num; j++)
	{
		float aux = 2.0 * M_PI * j / num;
		for (i = 0; i < num_aux; i++)
		{
			vertice_aux.x = perfil[i].x * cos(aux) + perfil[i].z * sin(aux);
			vertice_aux.z = -perfil[i].x * sin(aux) + perfil[i].z * cos(aux);
	...
	if (fabs(perfil[0].x) > 0.0 && tapa == 1)
	{
		...
		{
			cara_aux.y = j * num_aux;
			cara_aux.x = ((j + 1) * num_aux) % (vertices.size()-2);

			//caras.insert(caras.begin(),cara_aux);
			caras.push_back(cara_aux);
		}
	}
	...
	_semiEsfera::_semiEsfera(float radio, int n, int num)
	{
		...
		for (int i = 1; i <= n; i++)
		{
			v_aux.x = radio * cos(M_PI / 2 + (M_PI * i / n) / 2);
			v_aux.y = radio * sin(M_PI / 2 + (M_PI * i / n) / 2);
			perfil.push_back(v_aux);
		}
